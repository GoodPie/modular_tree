...
def create_default_leaf_object() -> Object:
    ...
    try:
        ...
        if collection is None:
            ...
            scene = bpy.context.scene if bpy.context.scene else (
                bpy.data.scenes[0] if bpy.data.scenes else None
            )
...
def create_leaves_distribution_node_group() -> NodeTree:
    ...
    leaf_socket = ng.interface.new_socket(name="Leaf Object", in_out='INPUT', socket_type='NodeSocketObject')
...
def distribute_leaves(ob: Object, leaf_object: Object | None = None) -> None:
    ...
    if not hasattr(mesh, 'attributes'):
        raise ValueError(
            f"Object '{ob.name}' has no mesh data. Cannot distribute leaves."
        )
