...
void TrunkFunction::execute(std::vector<Stem>& stems, int id, int parent_id)
{
	...
	Node firstNode{stem_direction, Geometry::get_orthogonal_vector(stem_direction), segment_length,
	               start_radius, id};
	...
	for (size_t i = 0; i < node_count; i++)
	{
		...
		Vector3 direction =
		    current_node->direction + Geometry::random_vec() * (randomness / (resolution + .001f));
		direction += Vector3(0, 0, up_attraction / (resolution + .001f));
		direction.normalize();
		float position_in_parent = 1;
		NodeChild child{Node{direction, firstNode.tangent, segment_length, radius, id},
		                position_in_parent};
